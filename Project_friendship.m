...
T = 100;
Tmod = 5;
sigma = 2; %radius of friendship attraction
...
for t = 1:T
    ...
    [a_network_x,a_network_y] = NetworkPositions(Friend_NetworkMatrix,agent_x, agent_y);
    
    E_mat = E_mat_0;
    
    for j = 1:length(a_network_x)
        E_Network = gauss_potential(lattice_size+2, a_network_x(j), a_network_y(j), sigma); %time consuming step
        E_mat = E_mat+E_Network;                                                            %Calculates a potential shift caused by the 
    end                                                                                     %individual agents.
